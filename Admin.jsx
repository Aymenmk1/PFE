...
function Sidebar({ active, onNav, collapsed, onToggle }) {
  const links = [
    ...
    { id: "dossiers",   icon: "📋", label: "Dossiers",        badge: DOSSIERS_ATTENTE.length },
...
function Dossiers() {
  const [dossiers, setDossiers]     = useState(DOSSIERS_ATTENTE);
            ...
            <>
              ...
              <div className="detail-section">
                ...
                <div className="detail-grid">
                  ...
                  <div><span>CIN</span><strong>{selected.cin}</strong></div>
              ...
              <div className="detail-section">
                ...
                <div className="doc-list">
                  <div className="doc-item"><span className="doc-item__icon">🪪</span>{selected.cin} <em>(CIN)</em> <button className="doc-btn">Voir</button></div>
                  <div className="doc-item"><span className="doc-item__icon">📄</span>{selected.residence} <button className="doc-btn">Voir</button></div>
                  <div className="doc-item"><span className="doc-item__icon">📸</span>{selected.photos} photo{selected.photos > 1 ? "s" : ""} téléversée{selected.photos > 1 ? "s" : ""} <button className="doc-btn">Galerie</button></div>
                </div>
              </div>
              ...
              <div className="detail-actions">
                <button className="adm-btn adm-btn--danger" onClick={() => rejeter(selected.id)}>
                  ✕ Rejeter le dossier
                </button>
                <button className="adm-btn adm-btn--success" onClick={() => valider(selected.id)}>
                  ✓ Valider & Publier
                </button>
              </div>
            </>
...
function FamillesPubliees() {
  const [familles, setFamilles] = useState(FAMILLES_PUBLIEES);
  ...
  const toggleStatut = (id) => {
    setFamilles((prev) =>
      prev.map((f) =>
        f.id === id
          ? { ...f, statut: f.statut === "active" ? "suspendue" : "active" }
          : f
      )
    );
  };
        ...
        <div className="adm-table">
          ...
          {liste.map((f) => (
            <div className="adm-table__row" key={f.id} style={{ gridTemplateColumns: "2fr 1fr 0.8fr 0.8fr 1fr 1.2fr 0.8fr 1.2fr" }}>
              ...
              <div>📍 {f.ville}</div>
              <div><span className="star-val">★ {f.note}</span></div>
              <div>{f.avis}</div>
              <div>{f.reservations}</div>
              <div><strong>{f.revenus.toLocaleString()} MAD</strong></div>
              <div><Badge statut={f.statut} /></div>
              <div className="row-actions">
                <button className="adm-btn-sm adm-btn-sm--outline" onClick={() => toggleStatut(f.id)}>
                  {f.statut === "active" ? "Suspendre" : "Réactiver"}
                </button>
                <button className="adm-btn-sm adm-btn-sm--ghost">Voir</button>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="section-content">
      <div className="kpi-grid" style={{ gridTemplateColumns: "repeat(4, 1fr)" }}>
        ...
        <StatCard icon="✅" label="Confirmées"             value={RESERVATIONS.filter(r=>r.statut==="confirmée").length}   color="#2E7D32" />
        <StatCard icon="⏳" label="En attente"             value={RESERVATIONS.filter(r=>r.statut==="en attente").length}  color="#F57C00" />
        ...
        <div className="adm-table">
          ...
          {liste.map((r) => (
            <div className="adm-table__row" key={r.id} style={{ gridTemplateColumns: "0.8fr 1.5fr 1.5fr 0.8fr 1fr 0.7fr 1fr 1fr" }}>
              ...
              <div>{r.voyageur} <span className="row-sub">({r.pays})</span></div>
              <div>{r.ville}</div>
...
export default function Admin() {
  ...
  const sections = { dashboard: Dashboard, dossiers: Dossiers, familles: FamillesPubliees, reservations: Reservations, paiements: Paiements, statistiques: Statistiques, moderation: Moderation };
  ...
  return (
    <div className={`admin-layout${collapsed ? " sidebar-collapsed" : ""}`}>
      ...
      <div className="admin-main">
        <Topbar section={section} notifs={DOSSIERS_ATTENTE.length} />
