...
import "./Visiteur.css";   // base design tokens (couleurs, polices, navbar, btns…)
import "./Famille.css";    // styles spécifiques famille
...
function DepotDossier() {
  ...
  const submit = () => {
    ...
    setSubmitted(true);
    // TODO: POST /api/families/dossier (multipart/form-data)
  };
  ...
  <div className="section-label">Candidature</div>
      ...
      <div className="depot-form">
            ...
            <div className="upload-zone">
              <label className="upload-label">
                ...
                <div>
                  ...
                  <div className="upload-label__sub">{form.fichierCIN || "PDF ou image — Max 5 Mo"}</div>
                </div>
              ...
              <label className="upload-label">
                ...
                <div>
                  ...
                  <div className="upload-label__sub">{form.fichierResidence || "Délivré par votre arrondissement — Max 5 Mo"}</div>
                </div>
        ...
        <div className="depot-form__nav">
          {step > 1 && (
            <button className="btn btn--outline" onClick={prev}>← Précédent</button>
          )}
          <div style={{ flex: 1 }} />
          {step < 3 && (
          ...
          {step === 3 && (
            <button className="btn btn--gold btn--lg" onClick={submit} disabled={!stepValid}>
...
function TableauDeBord() {
  ...
  const stats = {
    revenus_mois: 3024,
    reservations_actives: 4,
    note: 4.8,
    statut_dossier: "Certifiée",
  };
        ...
        {tab === "reservations" && (
          <div className="dash-table">
            <div className="dash-table__head">
              <div>Voyageur</div><div>Pays</div><div>Arrivée</div><div>Départ</div><div>Nuits</div><div>Total</div><div>Statut</div>
            </div>
            {RESERVATIONS_DEMO.map((r) => (
              <div className="dash-table__row" key={r.id}>
                <div><strong>{r.voyageur}</strong></div>
                <div>{r.pays}</div>
                <div>{r.arrivee}</div>
                <div>{r.depart}</div>
                <div>{r.nuits}</div>
                <div>{r.total} MAD</div>
                <div>
                  <span className={`badge badge--${r.statut === "confirmée" ? "ok" : "wait"}`}>{r.statut}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {tab === "dossier" && (
          <div className="dash-dossier">
            <div className="dash-dossier__statut">
              <span className="badge badge--ok">✓ Dossier certifié</span>
              <span style={{ marginLeft: 12, color: "var(--text-soft)", fontSize: "0.88rem" }}>Validé le 28 mars 2026</span>
            </div>
            <ul className="dash-checklist">
              <li className="ok">✓ CIN vérifiée</li>
              <li className="ok">✓ Certificat de résidence validé</li>
              <li className="ok">✓ Photos du logement approuvées</li>
              <li className="ok">✓ Annonce publiée — Visible publiquement</li>
              <li className="ok">✓ Coordonnées bancaires renseignées</li>
            </ul>
            <div className="dash-dossier__actions">
              <button className="btn btn--outline">Modifier l'annonce</button>
              <button className="btn btn--outline">Mettre à jour les photos</button>
            </div>
          </div>
        )}
      </div>
...
function Calendrier() {
  ...
  const [reserves, setReserves] = useState([5, 6, 7, 12, 13, 14, 15]);
  const [bloques, setBloques]   = useState([22, 23]);
  ...
  const togglePresent = (jour) => {
    if (reserves.includes(jour)) return; // pas modifiable
      ...
      <div className="cal-wrap">
        ...
        <div className="cal-grid">
          {cellules.map((j, i) => {
            ...
            const isReserve = reserves.includes(j);
            const isBloque  = bloques.includes(j);
